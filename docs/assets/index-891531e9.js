var R=Object.defineProperty;var N=(a,s,e)=>s in a?R(a,s,{enumerable:!0,configurable:!0,writable:!0,value:e}):a[s]=e;var c=(a,s,e)=>(N(a,typeof s!="symbol"?s+"":s,e),e);(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))t(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const o of n.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&t(o)}).observe(document,{childList:!0,subtree:!0});function e(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function t(i){if(i.ep)return;i.ep=!0;const n=e(i);fetch(i.href,n)}})();const G=[self.location.hostname,"fonts.gstatic.com","fonts.googleapis.com","cdn.jsdelivr.net"],F=a=>{var s=Date.now(),e=new URL(a.url);return e.protocol=self.location.protocol,e.hostname===self.location.hostname&&(e.search+=(e.search?"&":"?")+"cache-bust="+s),e.href};self.addEventListener("activate",a=>{a.waitUntil(self.clients.claim())});self.addEventListener("fetch",a=>{if(G.indexOf(new URL(a.request.url).hostname)>-1){const s=caches.match(a.request),e=F(a.request),t=fetch(e,{cache:"no-store"}),i=t.then(n=>n.clone());a.respondWith(Promise.race([t.catch(n=>s),s]).then(n=>n||t).catch(n=>{})),a.waitUntil(Promise.all([i,caches.open("pwa-cache")]).then(([n,o])=>n.ok&&o.put(a.request,n)).catch(n=>{}))}});class J extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"closed"}),this.routes={}}connectedCallback(){console.log("Connected!"),window.addEventListener("hashchange",()=>{this.checkRoute()})}addRoutes(s){this.routes=s,this.checkRoute()}changeRoute(s){if(console.log("Route changed:",s),s)this.shadow.innerHTML=this.routes[s]?this.routes[s].routingFunction():this.sendToNotFound();else{const e=Object.entries(this.routes).find(([t,i])=>i.type==="default");e?window.location.hash="#/"+e[0]:this.sendToNotFound()}}sendToNotFound(){Object.entries(this.routes).find(([e,t])=>t.type==="notFound")&&(window.location.hash="#/"+defaultRoute[0],this.changeRoute(defaultRoute[0]))}checkRoute(){const s=window.location.hash.slice(2);this.changeRoute(s)}}customElements.define("app-router",J);class d{constructor(){if(d._instance)return d._instance;this.listeners=[],d._instance=this}static get instance(){return d._instance||(d._instance=new d),d._instance}subscribe(s,e){this.listeners[s]||(this.listeners[s]=[]),this.listeners[s].push(e)}publish(s,e){this.listeners[s]&&this.listeners[s].forEach(t=>t(e))}}class y{constructor(s,e,t,i=[]){this.properties=s,this.layer=e,this.startingCoordinates=t,this.coordinatesArray=i,this.coordinatesArray.length===0&&this.coordinatesArray.push(t)}get id(){return this.layer.layer+this.startingCoordinates.latitude+this.startingCoordinates.longitude}static fromPoint(s,e,t){return new y(s,e,t)}static fromPolyline(s,e,t){return new y(s,e,t[0],t)}static fromPolygon(s,e,t,i){return new y(s,e,t,i)}}class u{constructor(){c(this,"_instance");if(u._instance)return u._instance;u._instance=this}static get instance(){return u._instance||(u._instance=new u),u._instance}getFeature(s,e){const t=this.getLayerName(s.id),i=this.getLayerByName(e,t),n=this.getRelevantProperties(s.id.properties,i.relevant_properties);let o,r;if(s.id.point){let l=s.id.position._value;r=this.checkCoordinates(l),o=y.fromPoint(n,i,r)}if(s.id.polyline){let l=s.id.polyline.positions._value;r=this.checkCoordinates(l),o=y.fromPolyline(n,i,r)}if(s.id.polygon){let l=s.id.polygon.hierarchy._value.positions;r=this.checkCoordinates(l);let b=this.getPolygonCenter(s.id.polygon);o=y.fromPolygon(n,i,b,r)}return o}getLayerName(s){let e;return e=s.properties.layerName._value,e}getLayerByName(s,e){for(const t in s){const i=s[t];if(Array.isArray(i)||typeof i=="object"){const n=this.getLayerByName(i,e);if(n)return n}else if(typeof i=="string"&&i.includes(e))return s}return null}getRelevantProperties(s,e){const t={};if(e){for(const i of e)if(s[i.property_name]){let n={property_name:i.property_name,display_name:i.display_name,type:i.type};typeof s[i.property_name]._value=="string"?n.value=s[i.property_name]._value:n.value=s[i.property_name]._value.value,t[i.property_name]=n}return t.raiseName=s.raiseName._value,t.layerName=s.layerName._value,t}}checkCoordinates(s){if(Array.isArray(s)){let e=[];return s.map(t=>{let i=this.cartesianToCartographic(t);e.push(i)}),e}else return this.cartesianToCartographic(s)}cartesianToCartographic(s){const e=Cesium.Cartographic.fromCartesian(s);let t=Cesium.Math.toDegrees(e.longitude),i=Cesium.Math.toDegrees(e.latitude);return t=parseFloat(t.toFixed(8)),i=parseFloat(i.toFixed(8)),{longitude:t,latitude:i}}getPolygonCenter(s){const e=s.hierarchy.getValue().positions;if(e&&e.length==0)return;let t=0,i=0;for(let r=0;r<e.length;r++){let l=Cesium.Cartographic.fromCartesian(e[r]);t+=Cesium.Math.toDegrees(l.longitude),i+=Cesium.Math.toDegrees(l.latitude)}let n=t/e.length;return{latitude:i/e.length,longitude:n}}createGeoJson(s){return{type:"FeatureCollection",features:s.map(i=>({type:"Feature",geometry:{type:"Point",coordinates:[i.startingCoordinates.longitude,i.startingCoordinates.latitude]},properties:i.properties}))}}}class x{constructor(s,e,t,i){this.name=s,this.features=e.map(n=>new y(n.properties,n.layer,n.startingCoordinates,n.coordinatesArray)),this.type=t,this.lastSelected=i}}class h{constructor(){c(this,"_data");if(h._instance)return h._instance;h._instance=this}static get instance(){return h._instance||(h._instance=new h),h._instance}getData(){this.data={};let s=JSON.parse(localStorage.getItem("selectedTags"));s?this.data.selectedTags=s:this.data.selectedTags=[];let e=JSON.parse(localStorage.getItem("routes"));if(e)this.data.routes=e.map(i=>new x(i.name,i.features,i.type,i.lastSelected));else{e=[];let i=new x("Default",[],"default",!0);e.push(i),localStorage.setItem("routes",JSON.stringify(e)),this.data.routes=e}let t=JSON.parse(localStorage.getItem("theme"));return t||(t=0),this.data.theme=t,console.log("Local storage:",this.data),this.data}}class p{constructor(){c(this,"CATEGORIES_URL","./json/categories.json");c(this,"_data");if(p._instance)return p._instance;p._instance=this}static get instance(){return p._instance||(p._instance=new p),p._instance}get data(){return this._data}set data(s){this._data=s}getData(){return this.data?Promise.resolve(this.data):this.fetchAppData(this.CATEGORIES_URL).then(s=>(this.data=s,s))}async fetchAppData(s){try{const e=await fetch(s).then(i=>i.json()),t=await Promise.all(e.categories.map(async i=>{const n=await Promise.all(i.groups.map(async o=>{try{const r=await fetch(o);if(r.ok)return r.json();throw new Error(`Errore durante il recupero dei dati da ${o}`)}catch(r){return console.error(r),null}}));return i.groups=n,i}));return{...e,categories:t}}catch(e){throw console.error("Errore durante il recupero dei dati JSON",e),e}}}class g{constructor(){c(this,"THEMES_URL","./json/themes.json");c(this,"_themes");if(g._instance)return g._instance;g._instance=this}static get instance(){return g._instance||(g._instance=new g),g._instance}get themes(){return this._themes}set themes(s){this._themes=s}getThemes(){return this.themes?Promise.resolve(this.themes):fetch(this.THEMES_URL).then(s=>s.json()).then(s=>(this.themes=s,s))}}class m{constructor(){if(m._instance)return m._instance;m._instance=this}static get instance(){return m._instance||(m._instance=new m),m._instance}getPosition(){return new Promise((s,e)=>{navigator.geolocation.getCurrentPosition(t=>{s(t)},t=>{e(t)})})}}class j extends HTMLElement{constructor(){super();c(this,"_data");c(this,"_position");c(this,"filterLayersByTags",(e,t)=>{let i=[];return e.categories.map(n=>{n.groups.map(o=>{o.layers.map(r=>{r.tags&&t.some(l=>r.tags.includes(l))&&i.push({...r,tags:[...r.tags]})})})}),i});this.shadow=this.attachShadow({mode:"closed"})}get data(){return this._data}set data(e){this._data=e}get position(){return this._position}set position(e){this._position=e}async connectedCallback(){let e=document.createElement("app-splash");this.shadow.append(e),this.data=await p.instance.getData(),this.position={};try{let o=await m.instance.getPosition();this.position.latitude=o.coords.latitude,this.position.longitude=o.coords.longitude}catch(o){console.error("Impossibile recuperare la posizione",o)}this.shadow.innerHTML+=`
...
`;class oe extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"closed"}),this.transitioner=null}connectedCallback(){Cesium.Ion.defaultAccessToken="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiI5MjY2YmYxNy1mNTM2LTRlOWYtYTUyZC01ZmY0NjBhNzllMWEiLCJpZCI6MTY5MDU3LCJpYXQiOjE2OTU4ODQ4NzB9.bN66rOR5h37xuKVsuUSYRSLOGJy-34IhH9S1hr4NOOE",this.viewer=new Cesium.Viewer(this.shadow,{baseLayerPicker:!1,geocoder:!1,timeline:!1,animation:!1,homeButton:!1,navigationInstructionsInitiallyVisible:!1,navigationHelpButton:!1,sceneModePicker:!1,fullscreenButton:!1,infoBox:!1,sceneMode:Cesium.SceneMode.SCENE2D,mapMode2D:Cesium.MapMode2D.ROTATE}),this.viewer.screenSpaceEventHandler.setInputAction(t=>{this.dispatchEvent(new CustomEvent("map-click",{detail:{movement:t}}))},Cesium.ScreenSpaceEventType.LEFT_CLICK),this.viewer.screenSpaceEventHandler.setInputAction(t=>{this.mouseOver(t)},Cesium.ScreenSpaceEventType.MOUSE_MOVE),d.instance.subscribe("customroute-load",t=>{this.loadCustomDataSource(t,"custom-route","yellow")}),d.instance.subscribe("feature-selected",t=>{const i=u.instance.createGeoJson([t]);this.loadCustomDataSource(i,"selected-feature","orangered")}),d.instance.subscribe("customroutecard-click",t=>{const i=u.instance.createGeoJson([t]);this.loadCustomDataSource(i,"selected-feature","orangered")});const s=document.createElement("style");s.innerHTML=ne,this.shadow.append(s);const e=document.createElement("link");e.setAttribute("rel","stylesheet"),e.setAttribute("href","./css/cesium.css"),this.shadow.append(e)}getEntity(s){const e=s.position;return this.viewer.scene.pick(e)}setCameraToPosition(s){let e=this.viewer.camera.positionCartographic;e.height>2e6?e.height=2e3:e.height;const t=Cesium.Cartesian3.fromDegrees(s.longitude,s.latitude,e.height);this.viewer.camera.flyTo({destination:t,orientation:{heading:Cesium.Math.toRadians(0),pitch:Cesium.Math.toRadians(-90),roll:0},duration:.5})}checkUserPin(s){this.viewer.entities.getById("user-pin")?this.updateUserPin(s):this.createUserPin(s)}createUserPin(s){this.viewer.entities.add({name:"user_pin",id:"user-pin",position:Cesium.Cartesian3.fromDegrees(s.longitude,s.latitude,0),point:{pixelSize:8,color:Cesium.Color.BLUE,outlineColor:Cesium.Color.WHITE,outlineWidth:1}})}updateUserPin(s,e){s.position=e}mouseOver(s){const e=s.endPosition,t=this.viewer.scene.pick(e);Cesium.defined(t)&&Cesium.defined(t.id)?document.body.style.cursor="pointer":document.body.style.cursor="default"}changeMapMode(){let s=this.viewer.scene.mode;s===Cesium.SceneMode.SCENE3D?this.viewer.scene.morphTo2D(0):s===Cesium.SceneMode.SCENE2D&&this.viewer.scene.morphTo3D(0)}changeTheme(s){for(let e=0;e<=2;e++)e===s?this.viewer.imageryLayers.get(e).show=!0:this.viewer.imageryLayers.get(e).show=!1}getImageryProvider(s,e,t){return new Cesium.WebMapTileServiceImageryProvider({url:s,layer:e,style:"default",format:"image/jpeg",maximumLevel:19,tileMatrixSetID:"default",credit:new Cesium.Credit(t)})}loadImageryProviders(s){s.forEach(e=>{const t=this.getImageryProvider(e.url,e.layer,e.credit);this.viewer.imageryLayers.addImageryProvider(t)})}async loadLayers(s){const e=s.map(t=>this.createlayer(t).then(i=>({layer:t,data:i})));await Promise.all(e).then(async t=>{this.viewer.dataSources._dataSources.map(n=>n.name).forEach(n=>{n!=="custom-route"&&this.viewer.dataSources.getByName(n).forEach(r=>this.viewer.dataSources.remove(r))}),await Promise.all(t.map(async n=>{const o=await n.data.layer;o.name=n.layer.layer,this.viewer.dataSources.add(o),this.styleEntities(o,n.layer.style)}))})}async createlayer(s){const e=`${s.layer_url_wfs}?service=WFS&typeName=${s.layer}&outputFormat=application/json&request=GetFeature&srsname=EPSG:4326`;return fetch(e).then(t=>t.json()).then(t=>this.createAdditionalProperties(t,s)).then(t=>({features:t.features,layer:Cesium.GeoJsonDataSource.load(t)})).catch(t=>{throw console.error(t),t})}createAdditionalProperties(s,e){return s.features=s.features.map((t,i)=>(t.properties.raiseName=e.name+" "+i,t.properties.layerName=e.layer,t)),s}async loadCustomDataSource(s,e,t){this.viewer.dataSources.getByName(e).forEach(o=>{this.viewer.dataSources.remove(o)});let n=await Cesium.GeoJsonDataSource.load(s);n.name=e,await this.viewer.dataSources.add(n),this.styleCustomDataSource(n,t)}styleCustomDataSource(s,e){s.entities.values.forEach(t=>{t.billboard=void 0,t.point=new Cesium.PointGraphics({pixelSize:16,color:Cesium.Color.fromCssColorString(e).withAlpha(.01),outlineColor:Cesium.Color.fromCssColorString(e),outlineWidth:3})})}styleEntities(s,e){let t="YELLOW",i="YELLOW",n=.5;e&&e.color&&(t=e.color.toUpperCase(),i=e.color.toUpperCase()),e&&e.opacity&&(n=e.opacity),s.entities.values.forEach(o=>{switch(!0){case Cesium.defined(o.polyline):o.polyline.material=Cesium.Color.fromCssColorString(t).withAlpha(parseFloat(n)),o.polyline.width=2;break;case Cesium.defined(o.billboard):o.billboard=void 0,o.point=new Cesium.PointGraphics({pixelSize:8,color:Cesium.Color.fromCssColorString(i).withAlpha(.5),outlineColor:Cesium.Color.fromCssColorString(i),outlineWidth:1});break;case Cesium.defined(o.polygon):o.polygon.material=Cesium.Color.fromCssColorString(t).withAlpha(parseFloat(n)),o.polygon.outlineColor=Cesium.Color.fromCssColorString(t).withAlpha(parseFloat(n));break}})}}customElements.define("app-cesium",oe);class q extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"closed"})}render(){switch(this.getAttribute("active-tab")){case"suggested-route":this.suggestedRouteContent.classList.add("active"),this.customRouteContent.classList.remove("active"),this.infoContent.classList.remove("active"),this.tabs.forEach(e=>{e.id==="suggested-route-tab"?e.classList.add("active-tab"):e.classList.remove("active-tab")});break;case"custom-route":this.suggestedRouteContent.classList.remove("active"),this.customRouteContent.classList.add("active"),this.infoContent.classList.remove("active"),this.tabs.forEach(e=>{e.id==="custom-route-tab"?e.classList.add("active-tab"):e.classList.remove("active-tab")});break;default:this.suggestedRouteContent.classList.remove("active"),this.customRouteContent.classList.remove("active"),this.infoContent.classList.add("active"),this.tabs.forEach(e=>{e.id==="info-tab"?e.classList.add("active-tab"):e.classList.remove("active-tab")});break}}connectedCallback(){this.shadow.innerHTML=`
            ...
            `,this.component=this.shadow.querySelector(".component"),this.header=this.shadow.querySelector(".header"),this.legend=this.shadow.querySelector(".legend"),this.name=this.shadow.querySelector(".name"),this.category=this.shadow.querySelector(".category"),this.tools=this.shadow.querySelector(".tools"),this.goToBtn=this.shadow.querySelector("app-goto"),this.addToRouteBtn=this.shadow.querySelector("app-add-to-route"),this.info=this.shadow.querySelector("app-info-panel"),this.goToBtn.addEventListener("go-to",t=>this.goTo(t.detail.coordinates)),this.addToRouteBtn.addEventListener("add-route",()=>{d.instance.publish("addtocustomroutebtn-click",this.feature)});const e=document.createElement("link");e.setAttribute("rel","stylesheet"),e.setAttribute("href","./css/tab.info.component.css"),this.shadow.append(e)}goTo(e){const t=`https://www.google.com/maps/dir/?api=1&destination=${e.latitude},${e.longitude}`;window.open(t,"_blank")}}customElements.define("app-tab-info",ae);class f{constructor(){c(this,"_instance");if(f._instance)return f._instance;f._instance=this}static get instance(){return f._instance||(f._instance=new f),f._instance}calculateDistance(s,e){const t=s.longitude-e.longitude,i=s.latitude-e.latitude;return Math.sqrt(t*t+i*i)}nearestInsertion(s,e){const t=[...s];let i=0,n=this.calculateDistance(e,t[0].startingCoordinates);for(let r=1;r<t.length;r++){const l=this.calculateDistance(e,t[r].startingCoordinates);l<n&&(n=l,i=r)}const o=[t.splice(i,1)[0]];for(;t.length>0;){n=Number.MAX_VALUE;let r;for(let l=0;l<t.length;l++){const b=this.calculateDistance(o[o.length-1].startingCoordinates,t[l].startingCoordinates);b<n&&(n=b,r=l)}o.push(t.splice(r,1)[0])}return o}}class re extends HTMLElement{constructor(){super();c(this,"_route");c(this,"_features");this.shadow=this.attachShadow({mode:"closed"}),this.features=[],this.shadow.innerHTML=`
            ...
            `,this.routeTitle=this.shadow.querySelector(".route-name"),this.list=this.shadow.querySelector(".list"),this.sortBtn=this.shadow.querySelector(".sort"),this.sortDialog=this.shadow.querySelector("app-sort-route-dialog"),this.editBtn=this.shadow.querySelector(".edit"),this.editDialog=this.shadow.querySelector("app-edit-route-dialog"),this.newBtn=this.shadow.querySelector(".new"),this.newDialog=this.shadow.querySelector("app-new-route-dialog"),this.saveBtn=this.shadow.querySelector(".save"),this.saveDialog=this.shadow.querySelector("app-save-route-dialog"),this.manageBtn=this.shadow.querySelector(".manage"),this.manageDialog=this.shadow.querySelector("app-manage-routes-dialog");const e=document.createElement("link");e.setAttribute("rel","stylesheet"),e.setAttribute("href","./css/tab.customroute.component.css"),this.shadow.append(e)}get features(){return this._features}set features(e){this._features=e}get route(){return this._route}set route(e){this._route=e}connectedCallback(){h.instance.getData().routes&&h.instance.getData().routes.forEach(t=>{t.lastSelected===!0&&(this.route=t,this.render())}),this.route.type==="default"&&(this.editBtn.disabled=!0),d.instance.subscribe("addtocustomroutebtn-click",e=>{if(!this.checkFeature(e))this.createCard(e);else{let i=this.features.findIndex(o=>o.id===e.id);this.removeCard(i),this.createCard(e);let n=document.createElement("app-snackbar");n.setAttribute("type","temporary"),n.setAttribute("text","Tappa già presente nel percorso"),document.body.append(n)}this.resetOrder()}),this.sortBtn.addEventListener("click",async()=>{let e;try{e=await m.instance.getPosition(),this.sortDialog.route=this.route,this.sortDialog.openDialog()}catch(t){console.error("Impossibile recuperare la posizione",t),d.instance.publish("no-position-found")}}),this.sortDialog.addEventListener("sort-route",async()=>{const e=await m.instance.getPosition(),t={longitude:e.coords.longitude,latitude:e.coords.latitude},i=f.instance.nearestInsertion(this.features,t);this.list.innerHTML="",i.reverse(),this._features=[],this.route.features=[],this.route.features=[...i],i.forEach(n=>this.createCard(n)),this.resetOrder()}),this.editBtn.addEventListener("click",()=>{this.editDialog.route=this.route,this.editDialog.openDialog()}),this.editDialog.addEventListener("edit-name",e=>{let t=h.instance.getData().routes,i;for(let n=0;n<t.length;n++)t[n].name===e.detail.oldName&&(t[n].name=e.detail.newName,i=t[n]);this.route=i,localStorage.setItem("routes",JSON.stringify(t)),console.log("Percorsi salvati",h.instance.getData().routes),this.render()}),this.editDialog.addEventListener("delete-route",e=>{let t=h.instance.getData().routes,i=[];i=t.filter(o=>o.name!==e.detail.name);let n;i.forEach(o=>{o.type==="default"&&(o.lastSelected=!0,n=o)}),localStorage.setItem("routes",JSON.stringify(i)),console.log("Percorsi salvati",h.instance.getData().routes),this.route=n,this.render()}),this.newBtn.addEventListener("click",()=>{this.newDialog.routes=h.instance.getData().routes,this.newDialog.openDialog()}),this.newDialog.addEventListener("create-route",e=>{let t=new x(e.detail.name,[],"user-route",!0),i=h.instance.getData().routes;i.push(t);for(let n=0;n<i.length;n++)i[n].name!==t.name&&(i[n].lastSelected=!1);this.route=t,localStorage.setItem("routes",JSON.stringify(i)),console.log("Percorsi salvati",h.instance.getData().routes),this.render()}),this.saveBtn.addEventListener("click",()=>{this.saveDialog.route=this.route,this.saveDialog.openDialog()}),this.saveDialog.addEventListener("save-route",()=>{this.route.features=this.features.reverse();let e=h.instance.getData().routes;for(let t=0;t<e.length;t++)e[t].name===this.route.name&&(e[t]=this.route);localStorage.setItem("routes",JSON.stringify(e)),console.log("Percorso salvato",h.instance.getData().routes)}),this.manageBtn.addEventListener("click",()=>{this.manageDialog.routes=h.instance.getData().routes,this.manageDialog.openDialog()}),this.manageDialog.addEventListener("load-route",e=>{let t=h.instance.getData().routes,i=e.detail.route;for(let n=0;n<t.length;n++)t[n].lastSelected=!1,t[n].name===i.name&&(t[n].lastSelected=!0,this.route=t[n]);localStorage.setItem("routes",JSON.stringify(t)),console.log("Percorso caricato",h.instance.getData().routes),this.render()})}checkFeature(e){return this.features.some(i=>i.id===e.id)}createCard(e){let t=document.createElement("app-tab-custom-route-card");t.feature=e,this.features.unshift(e),this.list.prepend(t),this.scrollLeft=0,t.addEventListener("remove-card",()=>{let i=this.features.findIndex(n=>n.id===e.id);this.removeCard(i),this.resetOrder()}),t.addEventListener("increase-order",()=>{let i=this._features.findIndex(r=>r.id===e.id),n=i-1,o=this.list.querySelectorAll("app-tab-custom-route-card");o[n]&&(this._features.splice(i,1),this._features.splice(n,0,e),this.list.insertBefore(o[i],o[n]),this.resetOrder(),console.log("Features",this._features))}),t.addEventListener("decrease-order",()=>{let i=this._features.findIndex(r=>r.id===e.id),n=i+1,o=this.list.querySelectorAll("app-tab-custom-route-card");o[n]&&(this._features.splice(i,1),this._features.splice(n,0,e),o[n].insertAdjacentElement("afterend",o[i]),this.resetOrder(),console.log("Features",this._features))})}removeCard(e){this.list.querySelectorAll("app-tab-custom-route-card")[e].remove(),this.features.splice(e,1)}resetOrder(){this.cards=this.list.querySelectorAll("app-tab-custom-route-card");let e=1;this.cards.forEach(i=>{i.setAttribute("order",e),e++});let t=u.instance.createGeoJson(this.features);d.instance.publish("customroute-load",t)}render(){this.list.innerHTML="",this._features=[],this.routeTitle.innerHTML=this.route.name,this.route.features.length>0&&this.route.features.forEach(e=>{this.createCard(e)}),this.resetOrder(),console.log("Percorso attuale:",this.route),this.route.type==="default"?this.editBtn.disabled=!0:this.editBtn.disabled=!1}}customElements.define("app-tab-custom-route",re);class D extends HTMLElement{constructor(){super();c(this,"_feature");c(this,"_order");this.shadow=this.attachShadow({mode:"closed"}),this.colorManager=new w}get feature(){return this._feature}set feature(e){this._feature=e}get order(){return this._order}set order(e){this._order=e}render(){this.num.innerHTML=this.order}connectedCallback(){this.shadow.innerHTML=`
            <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
            <div class="component">
            ...
            `,this.setAttribute("is-open",!1),this.close=this.shadow.querySelector(".close-icon"),this.info=this.shadow.querySelector(".info"),this.legend=this.shadow.querySelector(".legend"),this.name=this.shadow.querySelector(".name"),this.category=this.shadow.querySelector(".category"),this.tools=this.shadow.querySelector(".tools"),this.content=this.shadow.querySelector(".content"),this.playBtn=this.shadow.querySelector("app-play-info-btn"),document.addEventListener("expand-info",t=>{this.setAttribute("is-open",!0),this.feature=t.detail.feature}),this.close.addEventListener("click",()=>this.setAttribute("is-open",!1));const e=document.createElement("link");e.setAttribute("rel","stylesheet"),e.setAttribute("href","./css/info.drawer.component.css"),this.shadow.append(e)}attributeChangedCallback(e,t,i){i!=t&&t!=null&&e=="is-open"&&(i=="true"?this.openDrawer():this.closeDrawer())}openDrawer(){this.classList.remove("close"),this.classList.add("open")}closeDrawer(){this.classList.remove("open"),this.classList.add("close")}}c(H,"observedAttributes",["is-open"]);customElements.define("app-info",H);class B extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"closed"})}render(){}connectedCallback(){this.shadow.innerHTML=`
            <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
            `,this.button=document.createElement("button"),this.button.innerHTML=`
            <span class="icon">
                <span class="material-symbols-outlined">layers</span>
            </span>
            `,this.shadow.append(this.button),this.setAttribute("is-open","false");let s=this.getAttribute("is-open");this.btn=this.shadow.querySelector("button");const e=document.createElement("link");e.setAttribute("rel","stylesheet"),e.setAttribute("href","./css/bench.toggle.component.css"),this.shadow.append(e),this.btn.addEventListener("click",()=>{s=this.getAttribute("is-open")==="true",this.setAttribute("is-open",!s+"")})}attributeChangedCallback(s,e,t){if(t!=e&&s=="is-open"){const i=new CustomEvent("bench-toggle",{detail:{isOpen:t}});this.dispatchEvent(i)}}}c(B,"observedAttributes",["is-open"]);customElements.define("app-bench-toggle",B);class _ extends HTMLElement{constructor(){super();c(this,"_feature");c(this,"_isOpen");this.shadow=this.attachShadow({mode:"closed"}),this._isOpen=!1}get feature(){return this._feature}set feature(e){this._feature=e,this.render()}get isOpen(){return this._isOpen}set isOpen(e){this._isOpen=e}render(){this.content.innerHTML="";const e=this.feature.properties;let t=[];for(const i in e)if(e.hasOwnProperty(i)){const n=e[i];if(i=="raiseName"||i=="layerName")continue;if(n.value){const o=document.createElement("div");o.classList.add("topic");const r=document.createElement("h4");r.innerHTML=n.display_name,o.append(r);const l=document.createElement("p");l.innerText=n.value,o.append(l),this.content.append(o),t.push(i)}}if(t.length===0){this.style.display="none";return}else this.style.display="block";this.playBtn.addEventListener("read-info",()=>{const i=new SpeechSynthesisUtterance;i.lang="it";const n=this.shadow.querySelector(".content").innerHTML;i.text=n,window.speechSynthesis.speak(i)})}connectedCallback(){this.shadow.innerHTML=`
            <button>Leggi info</button>
            <div class="info">
                <app-play-info-btn></app-play-info-btn>
                <div class="content"></div>
            </div>
            `,this.setAttribute("is-open",this.isOpen),this.button=this.shadow.querySelector("button"),this.info=this.shadow.querySelector(".info"),this.playBtn=this.shadow.querySelector("app-play-info-btn"),this.content=this.shadow.querySelector(".content"),this.button.addEventListener("click",()=>{const t=JSON.parse(this.getAttribute("is-open"));this.setAttribute("is-open",!t+"")}),d.instance.subscribe("tab-maximize",t=>{this.setAttribute("is-open",t+"")});const e=document.createElement("link");e.setAttribute("rel","stylesheet"),e.setAttribute("href","./css/tab.info.panel.component.css"),this.shadow.append(e)}attributeChangedCallback(e,t,i){i!=t&&t!=null&&e=="is-open"&&(this.isOpen=JSON.parse(i),this.isOpen==!0?this.classList.add("open"):this.classList.remove("open"),this.isOpen==!0?this.button.innerHTML="Mostra meno":this.button.innerHTML="Leggi info",d.instance.publish("tab-maximize",this.isOpen))}}c(_,"observedAttributes",["is-open"]);customElements.define("app-info-panel",_);class ce extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"closed"})}connectedCallback(){this.shadow.innerHTML=`
            <div class="component">
                <img src="./images/RAISE_pictogram_nobg.svg">
                <div class="loader"></div>
            </div>
            `;const s=document.createElement("link");s.setAttribute("rel","stylesheet"),s.setAttribute("href","./css/splash.component.css"),this.shadow.append(s)}}customElements.define("app-splash",ce);class z extends HTMLElement{constructor(){super();c(this,"_route");this.shadow=this.attachShadow({mode:"closed"}),this.shadow.innerHTML=`
            <dialog>
                <div class="content">
            ...
            `,this.dialog=this.shadow.querySelector("dialog"),this.closeBtn=this.shadow.querySelector(".cancel"),this.deleteBtn=this.shadow.querySelector(".delete");const s=document.createElement("link");s.setAttribute("rel","stylesheet"),s.setAttribute("href","./css/dialog.empty-route.component.css"),this.shadow.append(s)}connectedCallback(){this.closeBtn.addEventListener("click",()=>this.closeDialog()),this.deleteBtn.addEventListener("click",()=>{this.closeDialog(),this.dispatchEvent(new CustomEvent("empty-route"))})}openDialog(){this.dialog.showModal?this.dialog.showModal():this.dialog.setAttribute("open","")}closeDialog(){this.dialog.close?this.dialog.close():this.dialog.removeAttribute("open")}}customElements.define("app-empty-route",le);class de extends HTMLElement{constructor(){super();c(this,"_route");this.shadow=this.attachShadow({mode:"closed"}),this.shadow.innerHTML=`
            <dialog>
                <div class="content">
                    <div class="message">
            ...
            `,this.dialog=this.shadow.querySelector("dialog"),this.routeName=this.shadow.querySelector(".route-name"),this.closeBtn=this.shadow.querySelector(".cancel"),this.saveBtn=this.shadow.querySelector(".save");const e=document.createElement("link");e.setAttribute("rel","stylesheet"),e.setAttribute("href","./css/dialog.save-route.component.css"),this.shadow.append(e)}get route(){return this._route}set route(e){this._route=e,this.render()}render(){this.routeName.innerHTML=this.route.name}connectedCallback(){this.closeBtn.addEventListener("click",()=>this.closeDialog()),this.saveBtn.addEventListener("click",()=>{this.closeDialog(),this.dispatchEvent(new CustomEvent("save-route"))})}openDialog(){this.dialog.showModal?this.dialog.showModal():this.dialog.setAttribute("open","")}closeDialog(){this.dialog.close?this.dialog.close():this.dialog.removeAttribute("open")}}customElements.define("app-save-route-dialog",de);class he extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"closed"}),this.shadow.innerHTML=`
            <dialog>
                <div class="content">
                    <div class="message">
            ...
            `,this.dialog=this.shadow.querySelector("dialog"),this.routeName=this.shadow.querySelector(".route-name"),this.closeBtn=this.shadow.querySelector(".cancel"),this.sortBtn=this.shadow.querySelector(".sort");const s=document.createElement("link");s.setAttribute("rel","stylesheet"),s.setAttribute("href","./css/dialog.sort-route.component.css"),this.shadow.append(s)}get route(){return this._route}set route(s){this._route=s,this.render()}render(){this.routeName.innerHTML=this.route.name}connectedCallback(){this.closeBtn.addEventListener("click",()=>this.closeDialog()),this.sortBtn.addEventListener("click",()=>{this.closeDialog(),this.dispatchEvent(new CustomEvent("sort-route"))})}openDialog(){this.dialog.showModal?this.dialog.showModal():this.dialog.setAttribute("open","")}closeDialog(){this.dialog.close?this.dialog.close():this.dialog.removeAttribute("open")}}customElements.define("app-sort-route-dialog",he);class O extends HTMLElement{constructor(){super();c(this,"_routes");this.shadow=this.attachShadow({mode:"closed"}),this.shadow.innerHTML=`
            <dialog>
                <div class="content">
                    <div class="message">
            ...
            `,this.dialog=this.shadow.querySelector("dialog"),this.closeBtn=this.shadow.querySelector(".close"),this.closeBtn.addEventListener("click",()=>this.closeDialog());const s=document.createElement("link");s.setAttribute("rel","stylesheet"),s.setAttribute("href","./css/dialog.no-position-error.component.css"),this.shadow.append(s)}openDialog(){this.dialog.showModal?this.dialog.showModal():this.dialog.setAttribute("open","")}closeDialog(){this.dialog.close?this.dialog.close():this.dialog.removeAttribute("open")}}customElements.define("app-no-position-dialog",pe);let me=()=>"<page-map></page-map>",be=()=>"<page-tags></page-tags>";const ge=document.querySelector("app-router"),fe={index:{routingFunction:be,type:"default"},map:{routingFunction:me,type:"map"}};ge.addRoutes(fe);
